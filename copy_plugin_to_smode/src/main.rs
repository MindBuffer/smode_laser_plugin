...
fn main() {
    ...
    let plugin_build_dir = Path::new(manifest_dir).parent().unwrap().join("plugin").join("build").join("Visual Studio 15 2017");

    dbg!(&plugin_build_dir);
    let release_build = Path::new(&plugin_build_dir).join("Release").join("Laser.dll");
    ...
    let debug_build = Path::new(&plugin_build_dir).join("Debug").join("Laser-debug.dll");
